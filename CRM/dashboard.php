...
<style>
:root{
  --bg:#060b18;--panel:#0b1222;--panel2:#0d1730;
  --line:rgba(148,163,184,.14);--text:#f7f9ff;--muted:#8e9bb5;
...
body{
  ...
  color:var(--text);
  background:
    radial-gradient(circle at 70% -10%,rgba(37,99,235,.18),transparent 30%),
...
.icon-btn:hover{border-color:rgba(96,165,250,.45);color:#fff}
.notif{
  position:absolute;
  right:-2px;top:-3px;
...
.rail a.active{
  color:#fff;
  background:linear-gradient(90deg,rgba(59,130,246,.22),rgba(37,99,235,.06));
...
.rail-user strong{
  display:block;font-size:10px;color:#e8eef9;
...
.eyebrow{
  font-size:10px;color:#6f80a0;text-transform:uppercase;
  letter-spacing:1.6px;font-weight:700;margin-bottom:7px;
}
.hero h1{font-size:26px;letter-spacing:-1px;font-weight:800;margin:0}
.hero p{font-size:12px;color:var(--muted);margin-top:7px}
.filters{display:flex;gap:8px;align-items:center}
.filter{
  height:38px;border:1px solid var(--line);
  background:rgba(10,17,33,.85);color:#cdd7e7;
  border-radius:11px;padding:0 12px;font-size:11px;outline:none;
}
.filter option{background:#0b1222}
.btn-add{
  height:38px;border:0;border-radius:11px;
  background:linear-gradient(135deg,#3b82f6,#6366f1);
  color:#fff;font-size:11px;font-weight:700;padding:0 15px;
...
.kpi{
  ...
  background:linear-gradient(145deg,rgba(15,27,50,.96),rgba(8,16,31,.96));
...
.kpi:after{
  ...
  right:-35px;bottom:-45px;background:rgba(59,130,246,.15);filter:blur(5px);
}
...
.kpi-icon.cyan{background:rgba(34,211,238,.12);color:#67e8f9}
.kpi-icon.amber{background:rgba(251,191,36,.12);color:#fcd34d}
.kpi-icon.red{background:rgba(251,113,133,.12);color:#fb7185}
.kpi-icon.green{background:rgba(52,211,153,.12);color:#6ee7b7}
.kpi-icon.purple{background:rgba(167,139,250,.12);color:#c4b5fd}
.kpi .number{font-size:24px;font-weight:800;letter-spacing:-1px;margin-top:10px}
.kpi .label{font-size:10px;color:#7f8ca5;margin-top:2px}
.trend{font-size:9px;color:#56d6b0}
.dashboard-grid{display:grid;grid-template-columns:minmax(0,1.7fr) minmax(290px,.8fr);gap:14px;margin-bottom:14px}
.panel{
  background:linear-gradient(145deg,rgba(12,23,43,.94),rgba(7,14,28,.96));
  border:1px solid var(--line);border-radius:17px;
  box-shadow:0 18px 45px rgba(0,0,0,.18);overflow:hidden;
}
.panel-head{
  height:58px;padding:0 18px;display:flex;align-items:center;
  justify-content:space-between;border-bottom:1px solid rgba(148,163,184,.10);
}
.panel-title{display:flex;align-items:center;gap:9px;font-size:13px;font-weight:700}
.panel-title i{color:#60a5fa}
.panel-sub{font-size:9px;color:#687791}
.panel-body{padding:17px}
.pipeline{display:grid;grid-template-columns:repeat(5,1fr);gap:8px}
.stage{
  min-height:125px;border:1px solid rgba(148,163,184,.12);
  background:rgba(5,12,25,.54);border-radius:13px;padding:12px;position:relative;
}
.stage:before{
  content:"";position:absolute;top:0;left:0;right:0;height:2px;
  background:var(--c);box-shadow:0 0 14px var(--c);
}
.stage .stage-name{font-size:9px;color:#8592a9;text-transform:uppercase;letter-spacing:.5px}
.stage .stage-num{font-size:22px;font-weight:800;margin-top:13px}
.stage .stage-meta{font-size:9px;color:#63718a;margin-top:5px}
.stage-bar{height:4px;background:#101c31;border-radius:5px;overflow:hidden;margin-top:14px}
...
.hot-item{
  ...
  align-items:center;padding:11px 0;border-bottom:1px solid rgba(148,163,184,.08);
}
.hot-item:last-child{border-bottom:0}
.machine{
  width:32px;height:32px;border-radius:9px;
  background:linear-gradient(135deg,#102b55,#0b1930);
  display:flex;align-items:center;justify-content:center;color:#60a5fa;
}
.hot-name{font-size:11px;font-weight:700}
.hot-desc{font-size:9px;color:#687791;margin-top:3px}
.hot-value{text-align:right;font-size:10px;font-weight:700}
.score{font-size:8px;color:#60a5fa;margin-top:3px}
.scorebar{width:64px;height:3px;border-radius:5px;background:#152238;margin-top:4px;overflow:hidden}
.scorebar span{display:block;height:100%;background:linear-gradient(90deg,#6366f1,#22d3ee);width:var(--score)}
.lower{display:grid;grid-template-columns:1.2fr .8fr;gap:14px}
.chart-wrap{height:260px}
.activity-list{padding:4px 17px 10px}
.activity{
  display:grid;grid-template-columns:30px 1fr auto;gap:10px;
  padding:12px 0;border-bottom:1px solid rgba(148,163,184,.08);
}
...
.act-title{font-size:10px;font-weight:700}
.act-desc{font-size:9px;color:#74829b;margin-top:3px}
.act-time{font-size:8px;color:#56657e;white-space:nowrap}
.empty{padding:30px;text-align:center;color:#66758f;font-size:11px}
.quick-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px;padding:17px}
.quick{
  border:1px solid rgba(148,163,184,.10);background:rgba(6,13,27,.55);
  border-radius:12px;padding:13px;text-decoration:none;color:#dce5f5;transition:.2s;
}
.quick:hover{border-color:rgba(59,130,246,.45);transform:translateY(-2px)}
.quick i{color:#60a5fa;font-size:13px}
.quick strong{display:block;font-size:10px;margin-top:9px}
.quick span{font-size:8px;color:#687791}
.footer{text-align:center;color:#44536c;font-size:9px;margin-top:22px}
