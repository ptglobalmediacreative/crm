...
<title>CRM Command Center - PT Ganda Elang Tangguh</title>
...
.shell{display:flex}.rail{width:68px;position:fixed;top:72px;bottom:0;left:0;background:rgba(5,10,21,.78);border-right:1px solid var(--line);display:flex;flex-direction:column;align-items:center;padding:18px 0;gap:9px;z-index:40}.rail a{width:40px;height:40px;border-radius:12px;color:#6e7d97;display:flex;align-items:center;justify-content:center;text-decoration:none;transition:.2s}.rail a:hover,.rail a.active{color:#fff;background:linear-gradient(135deg,rgba(59,130,246,.28),rgba(37,99,235,.10));box-shadow:0 0 22px rgba(37,99,235,.18)}.rail .spacer{flex:1}.content{margin-left:68px;width:calc(100% - 68px);padding:26px 28px 50px}.hero-row{display:flex;justify-content:space-between;align-items:flex-end;gap:20px;margin-bottom:22px}.eyebrow{font-size:10px;color:#6f80a0;text-transform:uppercase;letter-spacing:1.6px;font-weight:700;margin-bottom:7px}.hero h1{font-size:26px;letter-spacing:-1px;font-weight:800;margin:0}.hero p{font-size:12px;color:var(--muted);margin-top:7px}.filters{display:flex;gap:8px;align-items:center}.filter{height:38px;border:1px solid var(--line);background:rgba(10,17,33,.85);color:#cdd7e7;border-radius:11px;padding:0 12px;font-size:11px;outline:none}.filter option{background:#0b1222}.btn-add{height:38px;border:0;border-radius:11px;background:linear-gradient(135deg,#3b82f6,#6366f1);color:#fff;font-size:11px;font-weight:700;padding:0 15px;box-shadow:0 0 24px rgba(59,130,246,.25)}
...
@media(max-width:1200px){.kpis{grid-template-columns:repeat(3,1fr)}.topnav a{padding:9px 8px}.brand{min-width:190px}.search{width:150px}}@media(max-width:900px){.topnav{display:none}.brand{flex:1}.dashboard-grid,.lower{grid-template-columns:1fr}.pipeline{grid-template-columns:repeat(3,1fr)}}@media(max-width:650px){.topbar{padding:0 14px}.content{padding:20px 14px 40px}.rail{display:none}.content{margin-left:0;width:100%}.kpis{grid-template-columns:repeat(2,1fr)}.hero-row{align-items:flex-start;flex-direction:column}.filters{width:100%;flex-wrap:wrap}.filter{flex:1;min-width:130px}.pipeline{grid-template-columns:1fr 1fr}.brand{min-width:0}.brand div{display:none}.top-actions .search{display:none}}
</style>
</head>
<body>
<div class="app">
...
<?php if(in_array('sales_activity',$menuNames)): ?><a href="salesactivity.php">Leads</a><?php endif; ?>
<?php if(in_array('account_management',$menuNames)): ?><a href="account_management.php">Accounts</a><?php endif; ?>
<?php if(in_array('transaction_request',$menuNames)): ?><a href="transactionrequest.php">TR Request</a><?php endif; ?>
<?php if(in_array('produk',$menuNames)): ?><a href="produk.php">Products</a><?php endif; ?>
<?php if(in_array('delivery_order',$menuNames)): ?><a href="deliveryinstruction.php">Delivery</a><?php endif; ?>
</nav>
<div class="top-actions"><div class="search"><i class="fas fa-search"></i><input placeholder="Search customer..."></div><button class="icon-btn"><i class="far fa-bell"></i><span class="notif">!</span></button><div class="avatar"><?= strtoupper(substr($fullName,0,1)) ?></div></div>
</header>
<div class="shell">
<aside class="rail">
<a class="active" href="dashboard.php" title="Dashboard"><i class="fas fa-grid-2"></i></a>
<?php if(in_array('sales_activity',$menuNames)): ?><a href="salesactivity.php" title="Sales Activity"><i class="fas fa-chart-line"></i></a><?php endif; ?>
<?php if(in_array('account_management',$menuNames)): ?><a href="account_management.php" title="Accounts"><i class="fas fa-building"></i></a><?php endif; ?>
<?php if(in_array('transaction_request',$menuNames)): ?><a href="transactionrequest.php" title="Transaction Request"><i class="fas fa-file-signature"></i></a><?php endif; ?>
<?php if(in_array('produk',$menuNames)): ?><a href="produk.php" title="Products"><i class="fas fa-tractor"></i></a><?php endif; ?>
<?php if(in_array('delivery_order',$menuNames)): ?><a href="deliveryinstruction.php" title="Delivery"><i class="fas fa-truck-moving"></i></a><?php endif; ?>
<div class="spacer"></div><a href="logout.php" title="Logout"><i class="fas fa-power-off"></i></a>
</aside>
<main class="content">
<section class="hero-row"><div class="hero"><div class="eyebrow">PT Ganda Elang Tangguh · CRM Command Center</div><h1>Good morning, <?= htmlspecialchars($fullName) ?> 👋</h1><p>Monitor your sales pipeline, customer activities and dealership performance.</p></div><div class="filters"><?php if(!$isSalesRole): ?><select class="filter" id="filterSales" onchange="applyFilter()"><option value="0">All Sales</option><?php foreach($allSalesList as $s): ?><option value="<?= $s['id'] ?>" <?= ($filterSalesId==$s['id'])?'selected':'' ?>><?= htmlspecialchars($s['full_name']) ?></option><?php endforeach; ?></select><?php endif; ?><input class="filter" type="month" id="filterMonth" value="<?= htmlspecialchars($filterMonth) ?>" onchange="applyFilter()"><button class="btn-add" onclick="location.href='account_management.php'"><i class="fas fa-plus me-1"></i> Add Lead</button></div></section>
<section class="kpis">
